...
const Login = React.lazy(
  () => import("../../playtomic-frontend-task/src/views/Login")
);
const Matches = React.lazy(
  () => import("../../playtomic-frontend-task/src/views/Matches")
);
...
function AppAuthProvider(props: { children: ReactNode }) {
  ...
  type TokenSet = UnwrapPromise<
    NonNullable<AuthProviderProps["initialTokens"]>
  >;
  ...
  const handleAuthRefresh = useCallback<
    NonNullable<AuthProviderProps["onAuthChange"]>
  >(
    (tokens) => {
  ...
  return (
    <AuthProvider
      ...
      onAuthChange={handleAuthRefresh}
    >
      {props.children}
    </AuthProvider>
  );
